...
- **Brand lockup (top):** `BrandMark size="md" surface="dark"` + the wordmark "Koudou" in `Space Grotesk 700 @ 20px`, painted in **brand amber** (`text-sidebar-primary`, HSL `36 90% 55%`) so the wordmark visually rhymes with the mark's amber lower arm and the active-row amber bar — the rail's three brand-amber moments (mark arm → wordmark → active bar) form one chromatic chord. Wordmark uses `leading-none` + `pt-0.5` to optically center against the mark (which carries internal padding). In collapsed mode, only the mark renders, at `size="lg"`.
