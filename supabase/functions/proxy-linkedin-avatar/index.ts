...
//
// Testability: the request handler is built by `createHandler({ ... })`
// which accepts injectable dependencies (auth, storage admin, fetch, rate
// limiter). The default export wires the real implementations into
// `Deno.serve`. Tests construct a handler with fakes and exercise it via
// plain `Request` / `Response` objects — no network or DB required.

import { createClient, SupabaseClient } from "https://esm.sh/@supabase/supabase-js@2.45.0";
import { requireUser as defaultRequireUser } from "../_shared/auth.ts";
...
export const BUCKET = "linkedin-avatars";
...
export function pickExtension(url: string, contentType: string): string {
...
// ── Dependency types for the testable handler ──────────────────────────
// We deliberately type these as the *minimum surface area* the handler
// uses, so test fakes don't need to mock the entire Supabase SDK.

export type RequireUserFn = (
  req: Request,
...
export interface HandlerDeps {
  ...
  /** Pre-built admin client. Tests pass a fake; production passes a real one. */
  admin: Pick<SupabaseClient, "storage">;
  /** Upstream image fetcher — defaults to globalThis.fetch in production. */
  fetchFn: typeof fetch;
  checkRateLimit: RateLimitFn;
}

/**
 * Build the request handler. Exported separately from `Deno.serve` so tests
 * can inject fakes and assert on responses without a running runtime.
 */
export function createHandler(deps: HandlerDeps) {
  const { requireUser, admin, fetchFn, checkRateLimit } = deps;
  ...
  return async (req: Request): Promise<Response> => {
    ...
    try {
      ...
      const cached = existing?.find((o: { name: string }) => o.name.startsWith(`${hash}.`));
      ...
      if (forceRefresh) {
        ...
        const staleNames = (stale ?? [])
          .filter((o: { name: string }) => o.name.startsWith(`${hash}.`))
