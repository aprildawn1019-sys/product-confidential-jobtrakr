...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // NOTE: We deliberately do NOT extract or store the LinkedIn profile photo.
    // LinkedIn's CDN (`media.licdn.com`) returns 403 to any out-of-session
    // browser request, so the URLs we'd capture here would never render and
    // produced confusing "broken image" states. The UI shows initials for
    // every LinkedIn-imported contact instead.
    ...
    // Strategy 4: Direct fetch for JSON-LD or meta tags as a last resort.
